...
public:
    static constexpr UARTPreset USART1_115200_LOW{
        USART1_BASE,          7,
        RCC_APB2ENR_USART1EN, RCC_BASE + offsetof(RCC_TypeDef, APB2ENR),
        USART1_IRQn,          1476};
    ...
    static constexpr UARTPreset USART2_115200_LOW{
        USART2_BASE,           7,
        RCC_APB1ENR1_USART2EN, RCC_BASE + offsetof(RCC_TypeDef, APB1ENR1),
        USART2_IRQn,           1476};

    static constexpr UARTPreset USART3_115200_LOW{
        USART3_BASE,           7,
        RCC_APB1ENR1_USART3EN, RCC_BASE + offsetof(RCC_TypeDef, APB1ENR1),
        USART3_IRQn,           1476};
...
inline void UARTLow::Init()
{
    ...
    CLEAR_BIT(reinterpret_cast<USART_TypeDef *>(preset_.USARTx)->CR1,
              USART_CR1_UE);
    ...
    SET_BIT(reinterpret_cast<USART_TypeDef *>(preset_.USARTx)->CR1,
            USART_CR1_UE);
...
inline bool UARTLow::IsRxDone()
{
    return READ_BIT(reinterpret_cast<USART_TypeDef *>(preset_.USARTx)->ISR,
                    USART_ISR_RXNE);
}
inline bool UARTLow::IsTxDone()
{
    return READ_BIT(reinterpret_cast<USART_TypeDef *>(preset_.USARTx)->ISR,
                    USART_ISR_TC);
}
inline uint8_t UARTLow::GetRx()
{
    return reinterpret_cast<USART_TypeDef *>(preset_.USARTx)->RDR;
}
inline void UARTLow::SetTx(uint8_t byte)
{
    reinterpret_cast<USART_TypeDef *>(preset_.USARTx)->TDR = byte;
}
...
{
    SET_BIT(reinterpret_cast<USART_TypeDef *>(preset_.USARTx)->CR3,
            USART_CR3_DMAT);
}
