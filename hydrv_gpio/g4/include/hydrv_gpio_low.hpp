...
public:
    ...
    public:
        ...
        const uint32_t RCC_AHB2ENR_GPIOxEN_;
...
public:
    static constexpr GPIOPort GPIOA_port{GPIOA_BASE, RCC_AHB2ENR_GPIOAEN,
                                         GPIOA_inited_pins_};
    static constexpr GPIOPort GPIOB_port{GPIOB_BASE, RCC_AHB2ENR_GPIOBEN,
                                         GPIOB_inited_pins_};
    static constexpr GPIOPort GPIOC_port{GPIOC_BASE, RCC_AHB2ENR_GPIOCEN,
                                         GPIOC_inited_pins_};
    static constexpr GPIOPort GPIOD_port{GPIOD_BASE, RCC_AHB2ENR_GPIODEN,
                                         GPIOD_inited_pins_};
...
consteval inline GPIOLow::GPIOLow(const GPIOPort &GPIO_group, unsigned pin,
    ...
    : is_inited_(GPIO_group.inited_pins_[pin]),
      ...
      RCC_AHB2ENR_GPIOxEN_(GPIO_group.RCC_AHB2ENR_GPIOxEN_),
...
inline hydrolib::ReturnCode GPIOLow::Init(uint32_t altfunc = 0)
{
    ...
    EnableGPIOxClock_(RCC_AHB2ENR_GPIOxEN_);
...
inline void GPIOLow::EnableGPIOxClock_(const uint32_t RCC_AHB2ENR_GPIOxEN)
{
    __IO uint32_t tmpreg = 0x00U;
    SET_BIT(RCC->AHB2ENR, RCC_AHB2ENR_GPIOxEN); /* Delay after an RCC peripheral
                                                   clock enabling */
    tmpreg = READ_BIT(RCC->AHB2ENR, RCC_AHB2ENR_GPIOxEN);
    (void)tmpreg;
}
...
inline void GPIOLow::SetPinConfig_(uint32_t speed, bool is_open_drain,
                                   uint32_t push_pull)
{
    MODIFY_REG(reinterpret_cast<GPIO_TypeDef *>(GPIOx_)->OSPEEDR,
               output_speed_reg_mask_, speed);
    if (is_open_drain)
    {
        SET_BIT(reinterpret_cast<GPIO_TypeDef *>(GPIOx_)->OTYPER,
                output_type_reg_mask_);
    }
    else
    {
        CLEAR_BIT(reinterpret_cast<GPIO_TypeDef *>(GPIOx_)->OTYPER,
                  output_type_reg_mask_);
    }
    MODIFY_REG(reinterpret_cast<GPIO_TypeDef *>(GPIOx_)->PUPDR,
               push_pull_reg_mask_, push_pull);
}

inline void GPIOLow::SetModeOutput_()
{
    MODIFY_REG(reinterpret_cast<GPIO_TypeDef *>(GPIOx_)->MODER, mode_reg_mask_,
               mode_reg_value_output_);
}
...
inline void GPIOLow::SetModeAltfunc_(uint32_t altfunc)
{
    if (pin_ > 7)
    {
        MODIFY_REG(reinterpret_cast<GPIO_TypeDef *>(GPIOx_)->AFR[1],
                   altfunc_reg_high_mask_, GetAltfuncRegHighValue_(altfunc));
    }
    else
    {
        MODIFY_REG(reinterpret_cast<GPIO_TypeDef *>(GPIOx_)->AFR[0],
                   altfunc_reg_low_mask_, GetAltfuncRegLowValue_(altfunc));
    }

    MODIFY_REG(reinterpret_cast<GPIO_TypeDef *>(GPIOx_)->MODER, mode_reg_mask_,
               mode_reg_value_altfunc_);
}

inline void GPIOLow::SetModeAnalog_()
{
    MODIFY_REG(reinterpret_cast<GPIO_TypeDef *>(GPIOx_)->MODER, mode_reg_mask_,
               mode_reg_value_analog_);
}
