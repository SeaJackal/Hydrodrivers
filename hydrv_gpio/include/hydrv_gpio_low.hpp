...
public:
    ...
    inline hydrolib_ReturnCode InitAsTimer(uint32_t altfunc)
    {
        if (is_inited_)
        {
            return HYDROLIB_RETURN_FAIL;
...
private:
    static inline void EnableGPIOxClock_(const uint32_t RCC_AHB1ENR_GPIOxEN)
    {
        ...
        SET_BIT(RCC->AHB1ENR,
                RCC_AHB1ENR_GPIOxEN); /* Delay after an RCC peripheral clock
                                         enabling */
        tmpreg = READ_BIT(RCC->AHB1ENR, RCC_AHB1ENR_GPIOxEN);
        (void)tmpreg;
    }

    inline void SetPinConfig_(uint32_t speed, bool is_open_drain,
                              uint32_t push_pull)
    {
