...
private:
    static constexpr uint32_t PWR_REGULATOR_VOLTAGE_HIGH_PERF_RANGE =
        0x1 << PWR_CR1_VOS_Pos;
    static constexpr uint32_t PWR_REGULATOR_VOLTAGE_LOW_POW_RANGE =
        0x2 << PWR_CR1_VOS_Pos;
...
{
    volatile uint32_t tmpreg = 0x00U;
    MODIFY_REG(PWR->CR1, PWR_CR1_VOS, PWR_REGULATOR_VOLTAGE_HIGH_PERF_RANGE);
...
constexpr unsigned Clock::CalculateSystemClockMHz_(const ClockPreset &preset)
{
    ...
    return input_mhz * preset.N / ((static_cast<int>(preset.R) + 1) * 2) /
           preset.M;
}

constexpr uint32_t Clock::CalculatePLLCFGRValue_(const ClockPreset &preset)
{
    return preset.source | ((preset.M - 1) << RCC_PLLCFGR_PLLM_Pos) |
           (preset.N << RCC_PLLCFGR_PLLN_Pos) |
           (static_cast<uint32_t>(preset.R) << RCC_PLLCFGR_PLLR_Pos) |
           RCC_PLLCFGR_PLLREN;
}
